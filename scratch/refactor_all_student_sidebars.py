...
def refactor_file(filepath):
    ...
    def replace_logo(match):
        pre = match.group(1).replace('px-8', 'px-6 lg:px-8 relative')
        icon_div = match.group(2).replace('mr-3', 'mr-3 shrink-0')
        text = match.group(3).replace('class="', 'class="sidebar-text transition-opacity duration-300 ')
        post = match.group(4)
...
if __name__ == '__main__':
    for file in glob.glob('mockups/student/*.html'):
        refactor_file(file)
