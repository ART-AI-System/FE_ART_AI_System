    ...
    <div className="flex-1 flex overflow-hidden p-6 gap-6 h-[calc(100vh-6rem)]">
      ...
      <div className="flex-1 bg-white rounded-[24px] shadow-sm border border-gray-100 flex flex-col overflow-hidden relative">
        {activeRoomId ? (
          <>
            {/* Chat Header */}
            <div className="px-6 py-3.5 border-b border-gray-200 flex justify-between items-center bg-white z-10 shadow-2xs">
              <div className="flex items-center">
                <div className="relative mr-3">
                  <img src={otherMember?.avatar || `https://ui-avatars.com/api/?name=${encodeURIComponent(otherMember?.fullName || 'Group')}&background=0084FF&color=fff`} className="w-10 h-10 rounded-full border border-gray-200 object-cover" alt="Avatar" />
                  <span className="absolute bottom-0 right-0 w-3 h-3 bg-[#31A24C] border-2 border-white rounded-full"></span>
                </div>
                <div>
                  <h2 className="text-base font-bold text-[#050505] leading-tight">{otherMember?.fullName || (activeConversation?.type === 'group' ? 'Group Chat' : 'Unknown')}</h2>
                  <p className="text-[11px] font-semibold text-[#31A24C] flex items-center">Active now</p>
                </div>
              </div>
              <div className="flex items-center space-x-1">
                <button className="w-9 h-9 rounded-full hover:bg-gray-100 text-[#0084FF] flex items-center justify-center transition-colors">
            ...
            >
              {activeRoomId.startsWith('temp_') ? (
                <div className="flex flex-col items-center justify-center h-full text-gray-400 space-y-3">
                  <Smile className="w-12 h-12 text-[#0084FF]/40" />
                ...
                <>
                  {hasMore && (
                    <div className="flex justify-center mb-4">
                      <button onClick={() => loadMore()} className="bg-gray-100 hover:bg-gray-200 px-4 py-1 rounded-full text-[11px] font-bold text-gray-600 uppercase tracking-wider transition-all">
                  ...
                  {messages.map((msg) => {
                    ...
                    if (isMe) {
                      return (
                        <div key={msg._id} className="flex justify-end max-w-[70%] ml-auto mb-2">
                          <div className="flex flex-col items-end">
                            <div className="bg-[#0084FF] text-white px-4 py-2.5 rounded-[18px] rounded-br-[4px] text-[15px] font-normal leading-relaxed shadow-xs">
                              {msg.content}
                            </div>
                            <div className="flex items-center mt-1 mr-1">
                              <span className="text-[10px] font-semibold text-gray-400 mr-1">{time}</span>
                              {isRead ? <CheckCheck className="w-3.5 h-3.5 text-[#0084FF]" /> : <Check className="w-3.5 h-3.5 text-gray-400" />}
                            </div>
                          </div>
                        </div>
                      );
                      ...
                      return (
                        <div key={msg._id} className="flex justify-start max-w-[70%] mr-auto items-end space-x-2 mb-2">
                          <img src={otherMember?.avatar || `https://ui-avatars.com/api/?name=${encodeURIComponent(otherMember?.fullName || 'User')}&background=0084FF&color=fff`} className="w-7 h-7 rounded-full shrink-0 shadow-xs mb-5 object-cover" alt="Avatar" />
                          <div className="flex flex-col items-start">
                            <div className="bg-[#E4E6EB] text-[#050505] px-4 py-2.5 rounded-[18px] rounded-bl-[4px] text-[15px] font-normal leading-relaxed">
                              {msg.content}
                            </div>
                            <span className="text-[10px] font-semibold text-gray-400 mt-1 ml-1 block">{time}</span>
                          </div>
                        </div>
                      );
                    }
